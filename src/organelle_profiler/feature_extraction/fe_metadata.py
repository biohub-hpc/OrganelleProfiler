...
from ops_utils.data.bbox_utils import normalize_bbox
...
def _load_cells_metadata(dataset: OpsDataset, morphology_path: Path, debug_tile_count: int = None) -> pd.DataFrame:
        ...
        # Try to load cell_painting_linked CSVs first (Option B: primary source for CP experiments)
        cp_linked_list = []
        for well in wells:
            # link_cell_painting.py outputs well as "A/1/0" format -> cell_painting_linked_A_1_0.csv
            well_with_site = f"{well}/0"
            well_safe = well_with_site.replace("/", "_")
            cp_linked_path = dataset.results_fast / f"cell_painting_linked_{well_safe}.csv"
            if cp_linked_path.exists():
                cp_df = pd.read_csv(cp_linked_path)
                cp_df["well"] = well_with_site  # Format for store access: "A/1/0"
                cp_df["store_key"] = "pheno_assembled_v3"
                cp_linked_list.append(cp_df)

        if cp_linked_list:
            # Option B: Use cell_painting_linked as primary source
            cells_df = pd.concat(cp_linked_list, ignore_index=True)
            print(f"Using cell_painting_linked as primary source: {len(cells_df)} cells")
            ...
            if n_with_seg > n_unique_pairs:
                ...
                raise ValueError(
                    ...
                    f"  python -m ops_process.ops_analysis.data.link_cell_painting_slurm --experiment <exp>\n\n"
            ...
            invalid_bbox_count = cells_df["bbox"].isna().sum()
            if invalid_bbox_count > 0:
                raise ValueError(
                    f"Found {invalid_bbox_count} cells with missing/invalid bbox in linked_results. "
                    f"All cells must have valid bboxes for feature extraction."
                )
