...
# Split mode: GPU phase on GPU nodes, CPU phase on cheap CPU nodes
python -m organelle_profiler.feature_extraction.feature_extraction_slurm -e 94 --split

# Split mode with preview
python -m organelle_profiler.feature_extraction.feature_extraction_slurm -e 94 --split --preview 3

# Resume from a specific phase (split mode only) - runs that phase + downstream
python -m organelle_profiler.feature_extraction.feature_extraction_slurm -e 94 --split --resume-from spmd
python -m organelle_profiler.feature_extraction.feature_extraction_slurm -e 94 --split --resume-from merge
python -m organelle_profiler.feature_extraction.feature_extraction_slurm -e 94 --split --resume-from aggregate
...
FE_SLURM_PARAMS_BASE = {
    "slurm_mem": "450GB",            # 40GB RAM (~5GB per worker with 8 CPUs)
    "cpus_per_task": 64,             # 64 CPUs for hybrid GPU/CPU processing
    "timeout_min": 90,              # 90 minutes (1hr target + 50% buffer)
...
FE_GPU_SLURM_PARAMS = {
    "slurm_mem": "450GB",            # GPU nodes have 500GB-2TB; request generously since we're paying for GPU anyway
    "cpus_per_task": 48,             # 12 workers × 24 I/O threads; 48 fits on 64-core nodes
    "timeout_min": 480,             # 700K cells ~136min on A6000; 8hr for safety (partials load + aggregation add ~1hr)
    "slurm_partition": "gpu",
    "slurm_ntasks_per_node": 1,
    "slurm_array_parallelism": 8,    # Let all GPU batches run concurrently
    "slurm_gres": "gpu:1",
    "slurm_constraint": "a100_80|h100|h200", # a100_80|h100|h200
    ...
    for partition_idx in range(start_idx, len(escalation_order)):
        ...
        while time.time() - start_time < PENDING_TIMEOUT_SECS:
            ...
            if running:
                ...
                if not args.no_wait:
                    from ops_utils.hpc.slurm_batch_utils import _wait_for_jobs
...
def submit_spmd_cpu_phase(
    ...
    slurm_params = {
        "timeout_min": 10,  # 10 minutes per rank
        "slurm_mem": "4G",
        "cpus_per_task": 1,
        "slurm_partition": "cpu",
...
#SBATCH --job-name=fe_merge_combined
#SBATCH --partition=cpu
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=32
#SBATCH --mem=512G
#SBATCH --time=02:00:00
#SBATCH --output={log_path}/merge_combined_%j.out
#SBATCH --error={log_path}/merge_combined_%j.err
...
{conda_python} -m organelle_profiler.feature_extraction.spmd_cpu_merge \\
    ...
    --output-dir '{output_dir}'
"""
    merge_script_path = log_path / "merge_combined_array.sh"
    merge_script_path.write_text(merge_script)

    merge_result = subprocess.run(["sbatch", str(merge_script_path)], capture_output=True, text=True)
    if merge_result.returncode != 0:
        print(f"  ERROR: sbatch failed for merge job: {merge_result.stderr}")
        return {"success": False, "error": f"Merge sbatch failed: {merge_result.stderr}"}

    merge_job_id = merge_result.stdout.strip().split()[-1]
    print(f"  Merge Job ID: {merge_job_id}")

    if wait_for_completion:
        # Wait for merge to complete
        print(f"\n  Waiting for Merge ({merge_job_id}) to complete...")
        poll_interval = 15
        ...
        while True:
            ...
            try:
                sacct_result = subprocess.run(
                    ["sacct", "-j", merge_job_id,
                     "--format=JobID,State", "--noheader", "--parsable2"],
                    capture_output=True, text=True, timeout=10,
                )
                lines = [l for l in sacct_result.stdout.strip().split("\n") if l.strip()]
            except Exception as e:
            ...
            for line in lines:
                parts = line.split("|")
                if len(parts) >= 2 and parts[0] == merge_job_id:
                    merge_state = parts[1]

            elapsed_min = elapsed / 60
            print(f"  [{elapsed_min:.0f}m] Merge: {merge_state or 'UNKNOWN'}", end="\r")

            if merge_state == "COMPLETED":
                print(f"\n  Merge job COMPLETED after {elapsed_min:.1f} min")
                return {
                    "success": True,
                    "spmd_result": result,
                    "merge_job_id": merge_job_id,
                }
            elif merge_state in ("FAILED", "CANCELLED", "TIMEOUT", "OUT_OF_MEMORY"):
                print(f"\n  Merge job {merge_state} after {elapsed_min:.1f} min")
                return {
                    "success": False,
                    "error": f"Merge job {merge_state}",
                    "spmd_result": result,
                    "merge_job_id": merge_job_id,
                }

            if elapsed > 7200:
                print(f"\n  Timed out waiting for merge after {elapsed_min:.1f} min")
                return {
                    "success": False,
                    "error": "Timeout waiting for merge job",
                    "spmd_result": result,
                    "merge_job_id": merge_job_id,
                }
...
def submit_merge_job(
    ...
    """
    Submit merge job only (for --resume-from merge).

    Used when SPMD has already completed and we just need to run the merge phase.

    Parameters
    ----------
    batch_ids : list[str]
        List of batch IDs to merge
    experiment : str
        Experiment name
    output_dir : Path
        Output directory
    ...
    wait_for_completion : bool
        If True, wait for job to complete
...
def aggregate_batch_results(
    ...
    feature_prefixes = ["cell_", "cp_cell_", "network_"] + [f"{org}_" for org in organelle_names]
    ...
    guide_metadata_cols = ["sgRNA", "barcode", "gene_name", "gene_effect", "NCBI_ID"]
    ...
    # Gene-level aggregation (parallel across column chunks)
    gene_metadata_cols = ["gene_effect", "NCBI_ID"]
...
def submit_feature_extraction_jobs(
    ...
    """
    ds = OpsDataset(experiment)
    output_dir = ds.results_fast / "feature_extraction"
    ...
    cells_per_batch = getattr(args, 'cells_per_batch', DEFAULT_CELLS_PER_BATCH)
    ...
    morphology_path = ds.store_paths["pheno_assembled_v3"]
    # print check if icd.ops or icd.fast.ops and add nice spacing to clearly see the message
    print("\n" + "="*60)
    morphology_path_str = str(morphology_path)
    if "fast_ops" in morphology_path_str or "icd.fast" in morphology_path_str:
        print("Using icd.fast.ops - VAST filesystem (flash-based)")
    elif "intracellular_dashboard/ops" in morphology_path_str or "icd.ops" in morphology_path_str:
        print("Using icd.ops - Lustre filesystem")
    else:
    ...
    if split_mode:
        ...
        run_gpu = resume_from is None or resume_from == "gpu"
        run_spmd = resume_from in (None, "gpu", "spmd")
        run_merge = resume_from in (None, "gpu", "spmd", "merge")
        run_aggregate = True  # Always run aggregation at the end

        if resume_from:
            print(f"\n{'='*60}")
            print(f"RESUME MODE: Starting from '{resume_from}' phase")
            phases_to_run = []
        ...
        if run_spmd or run_merge:
            ...
            print(f"\n[Skipping SPMD/Merge phases - resuming from '{resume_from}']")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--split",
        action="store_true",
        help="Split into GPU + CPU phases (GPU on GPU nodes, CPU on cheap CPU nodes). "
             "GPU phase runs morphology/localization, then CPU phase runs network analysis.",
    )
    parser.add_argument(
        "--spmd-ntasks",
        type=int,
        default=1024,
        help="Number of MPI ranks for SPMD CPU phase in --split mode (default: 1024)",
    )
    parser.add_argument(
        "--agg-mem",
        type=str,
