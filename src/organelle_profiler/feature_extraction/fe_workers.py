...
if not _USE_GPU_FEATURES:
    ...
    print("[CPU] Using CPU feature extraction")
    ...
    for organelle_name, organelle_mask in organelles_with_masks.items():
        ...
        if organelle_name in network_organelles:
            ...
            if network_analysis_results:
                branch_df, network_summary_dict, per_object_network_df = network_analysis_results
...
def gpu_phase_worker(
    ...
    # 4.0GB/worker gives 17 workers on 80GB GPU (peak per-worker ~3-4GB)
    _PER_WORKER_VRAM_GB = 4.0
    _RESERVED_VRAM_GB = 10.0
    _MAX_WORKERS_PER_GPU = 24  # Cap per GPU to avoid compute contention
    try:
        import cupy as _cp
